...
public class CoreArmyLiteReborn
{
...
#nullable enable
    ...
    public void AggroMonStart(string? map = null)
    {
        ...
        int partySize = players.Length;
        // PartyOn();
        // Bot.Events.ExtensionPacketReceived += PartyManagement;
        ...
        foreach (int ID in _AggroMonIDs)
            AddMapIDs(
                GetMapIDs(Bot.Monsters.MapMonsters.Where(m => m.ID == ID || m.MapID == ID).ToList())
            );
    ...
    public void SmartAggroMonStart(string map, params string?[] monsters)
    {
        ...
        AggroMonCells(
            Core.ButlerOnMe() ? new[] { Bot.Player.Cell } : _SmartAggroMonCells.ToArray()
        );
        AggroMonStart();
    }
    ...
    public Option<string> player1 =
        new("player1", "Account #1", "Name of one of your accounts.", "");
    public Option<string> player2 =
        new("player2", "Account #2", "Name of one of your accounts.", "");
    public Option<string> player3 =
        new("player3", "Account #3", "Name of one of your accounts.", "");
    public Option<string> player4 =
        new("player4", "Account #4", "Name of one of your accounts.", "");
    public Option<string> player5 =
        new("player5", "Account #5", "Name of one of your accounts.", "");
    public Option<string> player6 =
        new("player6", "Account #6", "Name of one of your accounts.", "");
    public Option<string> player7 =
        new("player7", "Account #7", "Name of one of your accounts.", "");
    public Option<string> player8 =
        new("player8", "Account #8", "Name of one of your accounts.", "");
    public Option<string> player9 =
        new("player9", "Account #9", "Name of one of your accounts.", "");
    public Option<string> player10 =
        new("player10", "Account #10", "Name of one of your accounts.", "");

    public Option<int> packetDelay =
        new(
            "PacketDelay",
    ...
    public int getRoomNr()
    {
        ...
        {
            combinedDigits = (
                int.Parse(combinedDigits)
                * (DateTime.Now.Hour - DateTimeOffset.UtcNow.Hour)
                * DateTime.Today.Day
            ).ToString();
        }
        ...
        else
        {
            ...
            for (int i = 0; i < playersCount; i++) // Iterate through each player
            {
                ...
                if (username == p) // If current player is the one running the code
                    Core.Jump(cell); // Jump to the assigned cell
            }
        }
    }
        ...
        while (playerCount < dynamicPartySize)
        {
            if (Bot.Map.PlayerNames != null)
                foreach (var name in Bot.Map.PlayerNames)
                    if (
                        !playersWhoHaveBeenHere.Contains(name)
    ...
    public bool doForAll(bool randomServers = false)
    {
        ...
        if (Core.Username() != name)
        {
            ...
            Bot.Servers.Connect(
                randomServers
                    ? Bot
                        .Servers.ServerList.Where(x =>
                            !BlacklistedServers.Contains(x.Name.ToLower()) && !x.Upgrade && x.Online
                        )
                        .ToArray()[Bot.Random.Next(1, 5)]
        ...
        (string, string)[] readManager()
        {
            ...
            {
                Core.Logger(
                    $"There were no (sub-)folders named {Bot.Version} found in AppData/Local/Skua.Manager. Please set up your accounts in the Account tab in the Skua.Manager.exe",
                    "AccountManager",
                    true,
                    true
                );
            ...
            {
                Core.Logger(
                    $"Two or more (sub-)folders named {Bot.Version} were found in AppData/Local/Skua.Manager. Clean up AppData/Local/Skua and add your accounts to the account manager.",
                    "AccountManager",
                    true,
                    true
                );
            ...
            try
            {
                dyn = JsonConvert.DeserializeObject<dynamic[]>(
                    JsonConvert
                        .DeserializeObject<dynamic>(JsonConvert.SerializeXmlNode(xml))!
                        .configuration.userSettings["Skua.Manager.Properties.Settings"]
                        .setting.ToString()
                );
            }
            catch
            {
                Core.Logger(
                    $"Failed to parse account information from AppData/Local/Skua.Manager. Clean up AppData/Local/Skua and add your accounts to the account manager.",
                    "AccountManager",
                    true,
                    true
                );
            ...
            foreach (var d in dyn)
            {
                ...
                {
                    string[] accs = JsonConvert.DeserializeObject<string[]>(
                        d["value"].ArrayOfString["string"].ToString()
                    );
            ...
            {
                Core.Logger(
                    $"No accounts were found in the Skua.Manager's Account Manager. Please set up your accounts in the Skua.Manager and start up the bot again.",
                    "AccountManager",
                    true
                );
    ...
    )
    {
        ...
        while (!Bot.ShouldExit)
        {
            ...
            if (!tryGoto(playerName))
            {
                ...
                Core.Logger(
                    $"Could not find {playerName}. Check if \"{playerName}\" is in the same server with you.",
                    "tryGoto"
                );
                if (b_shouldHibernate)
                    Core.Logger(
                        $"The bot will now hibernate and try to /goto to {playerName} every {hibernateTimer} seconds.",
                        "tryGoto"
                    );
            ...
            if (
                !Bot.Combat.StopAttacking
                && Bot.Monsters.CurrentMonsters.Any(m => Core.IsMonsterAlive(m))
            )
                PriorityAttack("*");
    ...
    private bool tryGoto(string userName)
    {
        ...
        if (
            Bot.Map.PlayerExists(userName)
            && Bot.Map.TryGetPlayer(userName, out PlayerInfo? playerObject)
            && playerObject != null
    ...
    private void LockedMaps()
    {
        ...
        if (File.Exists(Path.Combine(CoreBots.ButlerLogDir, b_playerName + ".txt")))
        {
            ...
            string? targetMap = File.ReadAllLines(
                    Path.Combine(CoreBots.ButlerLogDir, b_playerName + ".txt")
                )
                .FirstOrDefault();
        ...
        string[] EventMaps = { "yoshino" };

        string[] VerusMaps = { "seavoice" };
        ...
        {
            Core.Logger(
                $"[{(maptry.ToString().Length == 1 ? "0" : "")}{maptry++}/{mapCount}] Searching for {b_playerName} in /{map}",
                "LockedZoneHandler"
            );
            Core.Join(map);
        ...
        foreach (var mapInfo in levelLockedMaps)
        {
            ...
            Core.Logger(
                $"[{(maptry.ToString().Length == 1 ? "0" : "")}{maptry++}/{mapCount}] Searching for {b_playerName} in /{mapInfo.Map}",
                "LockedZoneHandler"
            );
        ...
        Core.Join("whitemap");
        Core.Logger(
            $"Could not find {b_playerName} in any of the maps within the LockedZoneHandler.",
            "LockedZoneHandler"
        );
        if (b_shouldHibernate)
            Core.Logger(
                $"The bot will now hibernate and try to /goto to {b_playerName} every {b_hibernationTimer} seconds",
                "LockedZoneHandler"
            );
    ...
    public void PriorityAttack(string attNoPrio)
    {
        ...
        {
            var _mon = Bot.Monsters.CurrentMonsters.Find(m =>
                m.Name.Trim().ToLower() == mon.ToLower() && Core.IsMonsterAlive(m)
            );
            if (_mon != null)
            {
    ...
    private async void MapNumberParses(string map)
    {
        ...
        if (
            !Int32.TryParse(Bot.Map.FullName.Split('-').Last(), out int mapNr)
            || map == b_prevRoom
